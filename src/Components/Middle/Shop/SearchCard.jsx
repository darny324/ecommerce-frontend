...
const SearchCard = (prop) => {
  ...
  return (
    ...
    className="flex items-center gap-4 px-4">
      <div className="w-32 h-32 shrink-0"><img src={product.images[product.main_image_option]} className="w-full h-full object-contain" /></div>
      <div>
        <h2 className="text-lg md:text-xl line-clamp-2 overflow-hidden">{product.name}</h2>
        <p className="text-gray-400">{product.attributes.brand}</p>
        ...
        hover:border-none hover:bg-black hover:text-white">See More</Link>
      </div>
    </motion.div>
  )
}
